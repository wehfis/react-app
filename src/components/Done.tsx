...
const Done:React.FC<Props> = ({ todos, set_todos, handleDelete, handleUpdate }) =>{
  ...
  const handleEditSubmit = (e: React.FormEvent) => {
    ...
    return (
      ...
      {(provided) => (
        <div className="ticket3" {...provided.droppableProps} ref={provided.innerRef}>
          <span className="text_ticket3">Done</span>
          <div className="task_grid">
            {todos.map((todo, index) => (
              <Draggable key={todo.id} draggableId={todo.id} index={index}>
                {(provided) => (
                  <div
                    className="task_ticket3"
