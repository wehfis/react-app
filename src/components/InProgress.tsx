...
const InProgress:React.FC<Props> = ({ todos, set_todos, handleDelete, handleUpdate }) =>{
  ...
  const handleEditSubmit = (e: React.FormEvent) => {
    ...
    return (
      <Droppable droppableId="inprogress">
        {(provided) => (
          <div className="ticket2" {...provided.droppableProps} ref={provided.innerRef}>
            <span className="text_ticket2">In Progress</span>
            <div className="task_grid">
              {todos.map((todo, index) => (
                <Draggable key={todo.id} draggableId={todo.id} index={index}>
                  {(provided) => (
                    <div
                      className="task_ticket2"
